#!/usr/bin/env bash
# Modul 20-locale-keyboard — Locale, Tastaturlayout, Timezone
# shellcheck shell=bash
module_run() {
  local manage="${LOCALE_MANAGE:-auto}"

  # --- Timezone (i.d.R. unkritisch, eigener Schalter über TIMEZONE) ---
  if [[ -n "${TIMEZONE:-}" ]]; then
    ...
    if [[ "$cur" != "$TIMEZONE" ]]; then
      ...
      log_info "20-locale-keyboard: Timezone bereits $TIMEZONE."
    fi
  fi
  ...
  if [[ -n "${LOCALE:-}" ]]; then
    ...
    log_info "20-locale-keyboard: LANG=$LOCALE gesetzt."
  fi
  ...
  if [[ -n "${KEYMAP:-}" ]]; then
    ...
    log_info "20-locale-keyboard: KEYMAP=$KEYMAP gesetzt."
  fi
}
